...
static ColorState colors = {
	...
	0, 0 // current colors (initialized to defaults later)
};

static PsvDebugScreenFont *psvDebugScreenFontCurrent = &psvDebugScreenFont;
static int font_bits_patched = 0;
...
#else
    // reverse the bits in the font once to fix glyph orientation
	if (!font_bits_patched) {
		for (int i = 0; i < 768; i++) {
			char reversed = 0;
			for (int j = 0; j < 8; j++) {
				if (F->glyphs[i] & (1 << j)) {
